// BookingModal.tsx - Fixed version with correct Provider interface
import React, { useState } from 'react';
import { X, Calendar, Clock, MapPin, DollarSign, User, Mail, Phone, Star, Shield, Award } from 'lucide-react';

// Updated Provider interface to match the one from ProvidersList
interface Provider {
  _id?: string;
  id: string;
  name: string;
...
const BookingModal: React.FC<BookingModalProps> = ({
  ...
  const [formData, setFormData] = useState<BookingFormData>({
    ...
    budget: provider.priceRange || `₦${provider.hourlyRate || 0}/hr`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const bookingData = {
        ...
        description: formData.description || `Booking for ${formData.serviceType}`,
