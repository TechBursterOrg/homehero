...
// BookingModal Component with Payment Integration
...
const BookingModal: React.FC<BookingModalProps> = ({
  ...
  const extractAmountFromBudget = (budget: string): number => {
    if (!budget) return provider.hourlyRate || 1000;
    const numericString = budget.replace(/[^\d.]/g, '');
    return parseFloat(numericString) || provider.hourlyRate || 1000;
  };
  ...
  const providerLocation = provider.address || `${provider.city}, ${provider.state}`;
