...
const LocationSearch: React.FC<LocationSearchProps> = ({
  ...
  // Function to find broader matches when exact location isn't found
  const findBroaderMatches = (query: string) => {
  const queryLower = query.toLowerCase();
  const matches = [];
  
  // Special handling for Ikeja and Lagos searches
  if (queryLower.includes('ikeja')) {
    // Return all Ikeja areas
    const ikejaAreas = nigerianLocations.filter(loc => 
      loc.address.toLowerCase().includes('ikeja')
    );
    matches.push(...ikejaAreas);
  }
  
  if (queryLower.includes('lagos') && !queryLower.includes('ikeja')) {
    // Return popular Lagos areas
    const lagosAreas = nigerianLocations.filter(loc => 
      loc.address.toLowerCase().includes('lagos') && !loc.address.toLowerCase().includes('ikeja')
    ).slice(0, 10);
    matches.push(...lagosAreas);
  }
  
  // If we still don't have matches, try the original logic
  if (matches.length === 0) {
    // Original matching logic from your component
    const citiesAndStates = nigerianLocations.filter(loc => 
      loc.type === 'city' || loc.type === 'state'
    );
    
    for (const location of citiesAndStates) {
      const locationName = location.address.toLowerCase();
      if (locationName.includes(queryLower) || queryLower.includes(locationName.split(',')[0])) {
      ...
      <div className="relative">
        ...
        {searchQuery && (
          <button
            onClick={() => {
              setSearchQuery('');
              onLocationSelect({ address: '', coordinates: [0, 0] });
            }}
